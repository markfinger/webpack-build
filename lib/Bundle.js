...
Bundle.prototype.getConfig = function getConfig(cb) {
  ...
  if (this.opts.watchConfig && !this.watchingConfig && _.isString(this.opts.config)) {
    ...
    this.watchFile(this.opts.config, function() {
      if (this.opts.logger) {
        this.opts.logger.info('Webpack: config file change detected, stopping watcher for ' + this.opts.config);
      }
  ...
  this.getConfig(function(err, config) {
    ...
    if (this.opts.logger) {
      compiler.plugin('done', function(stats) {
        if (stats.hasErrors()) {
          this.opts.logger.error('Webpack: compilation error', stats.compilation.errors[0].stack);
        } else {
          var message = 'Webpack: compilation completed';
          if (this.opts.config) {
            message += ' for ' + this.opts.config;
          }
        ...
        if (stats.hasErrors()) {
          ...
          if (this.opts.watch) {
            this.onceWatcherDone(function(err) {
              ...
              this.cache.set(this.opts.config, entry, true);
            }.bind(this));
          } else {
            this.cache.set(this.opts.config, entry);
          }
        }
      }.bind(this));
    }
    ...
    if (this.opts.logger) {
      if (this.opts.config) {
        this.watcher.onInvalid(function() {
          this.opts.logger.info('Webpack: watcher invalidated ' + this.opts.config);
        }.bind(this));
      }
