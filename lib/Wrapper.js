...
Wrapper.prototype.getConfig = function getConfig(cb) {
  ...
  if (_.isString(this.config)) {
    ...
    if (this.opts.watchConfig && !this.watchingConfig) {
  ...
  if (this.config && this.opts.builds) {
    var buildErr = null;

    this.opts.builds.forEach(function(build) {
      var found = false;

      if (build in builds) {
        found = true;
        this.config = builds[build](this.config, this.opts);
      }

      if (this.config.builds && build in this.config.builds) {
        found = true;
        this.config = this.config.builds[build](this.config, this.opts);
      }

      if (!found) {
        var available = Object.keys(builds);

        if (this.config.builds) {
          available.concat(Object.keys(this.config.builds));
        }

        buildErr = new Error('Unknown build "' + build + '". Available builds: ' + available.join(', '));
      }
    }, this);
