...
var webpackWrapper = function webpackWrapper(opts, cb) {
  ...
  if (!wrapper) {
    wrapper = new Wrapper(opts, null, cache, cacheKey);
    wrappers.add(wrapper);
  }

  if (cacheKey && cache && !cache.updated[cacheKey]) {
    cache.get(cacheKey, function(err, entry) {
      if (err || !entry) {
        return wrapper.onceDone(cb);
      }

      if (opts.logger) {
        opts.logger.info('Webpack: serving cached output for key "' + cacheKey + '" in cache file ' + cacheFile);
      }
