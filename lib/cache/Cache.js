...
var Cache = (function () {
  ...
  _createClass(Cache, [{
    ...
    value: function get(cb) {
      ...
      var requiredProps = ['startTime', 'fileDependencies', 'stats', 'config', 'buildHash', 'dependencies'];
      ...
      _fs2['default'].stat(configFile, function (err, stats) {
        ...
        _async2['default'].each(data.fileDependencies, function (filename, cb) {
          ...
          if (err) {
            _this.logger('cache retrieval error: ' + err.message);
            return cb(err);
          }

          _this.logger('serving cached data');
          cb(null, data);
        });
      });
    }
